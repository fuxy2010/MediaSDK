...
public class OneShotLocation extends BroadcastReceiver {
	...
	public static void receive(Context context, Intent intent) {
    	...
    	if (SipdroidReceiver.mContext == null) SipdroidReceiver.mContext = context;
    	loc = (Location)intent.getParcelableExtra(LocationManager.KEY_LOCATION_CHANGED);
    	if (loc != null)
    	{
    		//fym SipdroidReceiver.pos(false);
    		SipdroidReceiver.url("lat="+loc.getLatitude()+"&lon="+loc.getLongitude()+"&rad="+loc.getAccuracy());
    	}
