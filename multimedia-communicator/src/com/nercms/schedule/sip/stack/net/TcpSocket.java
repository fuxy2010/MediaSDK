...
public class TcpSocket {
	...
	//Android2.3.3下不可用 static boolean lock;
...
//		socket = new Socket(ipaddr.getInetAddress(), port); modified
		socket = new Socket();
		//Android2.3.3下不可用 if (lock) throw new java.io.IOException();
		//Android2.3.3下不可用 lock = true;
		try
		{
			socket.connect(new InetSocketAddress(ipaddr.toString(), port), Thread.currentThread().getName().equals("main")?1000:10000);
			socket.setTcpNoDelay(true);//fym
		}
		catch (java.io.IOException e)
		{
			//Android2.3.3下不可用 lock = false;
			throw e;
		}
		//Android2.3.3下不可用 lock = false;
	}
