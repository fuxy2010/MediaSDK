...
package com.nercms.schedule.sip.stack.sip.provider;
...
import com.nercms.schedule.sip.stack.net.*;
import com.nercms.schedule.sip.stack.sip.header.*;
import com.nercms.schedule.sip.stack.sip.message.Message;
import com.nercms.schedule.sip.stack.sip.address.*;
import com.nercms.schedule.sip.stack.tools.Configure;
import com.nercms.schedule.sip.stack.tools.Configurable;
import com.nercms.schedule.sip.stack.tools.Parser;
import com.nercms.schedule.sip.stack.tools.Random;
import com.nercms.schedule.sip.stack.tools.Log;
import com.nercms.schedule.sip.stack.tools.LogLevel;
import com.nercms.schedule.sip.stack.tools.RotatingLog;
import com.nercms.schedule.sip.stack.tools.SimpleDigest;
import com.nercms.schedule.sip.stack.tools.DateFormat;

import java.util.Hashtable;
import java.io.IOException;
import com.nercms.schedule.sip.stack.tools.HashSet;
import com.nercms.schedule.sip.stack.tools.Iterator;
 ...
 * SipProvider实现了SIP的传输层，即发送和接受SIP消息的功能。消息由SipProviderListener中定义的回调函数接收。
 * <p>
 ...
 * SipProvider同时通过使用SIP interface identifiers和SipProviderListener中定义onReceivedMessage回调函数的实现了复用/解复用服务
 * <p>
 ...
 * 一个SipProviderListener可以通过addSipProviderListener(id,listener)函数被添加到一个SipProvider中，id是listener所绑定的SIP interface的ID，listener是所收到的消息的接收方。
 * SIP Interface的ID表明了listener接受的消息的类型。ID与特定的SipProvider一起形成一个完成SIP Service Access Point（SAP）地址或ID，供接收方解复用SIP消息用。
 * 上述ID是下列三种类型之一：transaction_id, dialog_id, or method_id，分别表明该消息是某个事务/对话/方法中的消息，当然也可用使用ANY来表明非事务/对话/方法中的消息。
 *  *  * <p>
 ...
 * fired.
 * 受到一个消息时，SipProvider首先尝试查找一个匹配的事务（listener），然后尝试查找匹配的对话（listener），再后查找匹配的方法（listener），最后为默认listener（即ANY类型）。
 * 找到匹配的SipProviderListener后，调用onReceivedMessage函数。
 * <p>
 ...
 * 注意：在查找匹配的事务、对话和方法失败时，不会产生482(Loop Detected)响应。
 */
public class SipProvider implements Configurable, TransportListener,
	...
	//fym
	private boolean over_tcp = false;
	public SipProvider(String via_addr, int port, int index)
	{
		//fym
		over_tcp = (0 == (index % 2)) ? true : false;
		transport_protocols = new String[1];
		transport_protocols[0] = (true == over_tcp) ? PROTO_TCP : PROTO_UDP;
	 ...
	 */
	/*fym public SipProvider(String via_addr, int port, String[] protocols,
			String ifaddr) {
	 ...
	 * file.
	 */
	/*fym public SipProvider(String file) {
		if (!SipStack.isInit())
			SipStack.init(file);
	...
	private void init(String viaddr, int port, String[] protocols, String ifaddr)
	{
		...
		host_port = port;//port为0则自动分配端口
		//android.util.Log.v("JSON", "1 host_port: " + host_port);
		if (host_port < 0) // modified
		{
			host_port = SipStack.default_port;
			//android.util.Log.v("JSON", "2 host_port: " + host_port);
		}
		host_ipaddr = null;
		...
		for (int i = 0; i < transport_protocols.length; i++)
		{
			...
			android.util.Log.v("Baidu", "transport_protocols " + transport_protocols[i] + " " + transport_protocols.length);
		}
	...
	private void startTrasport() {
		// start udp
		//android.util.Log.v("JSON", "6 host_port: " + host_port);
		if (transport_udp) {
			try {
				if (host_ipaddr == null)
				...
				//android.util.Log.v("JSON", "3 host_port: " + host_port);
		...
		if (transport_tcp) {
			try {
				...
				//android.util.Log.v("JSON", "4 host_port: " + host_port);
	...
	public void parseLine(String line) {
		...
		if (attribute.equals("host_port")) {
			...
			//android.util.Log.v("JSON", "5 host_port: " + host_port);
			return;
		}
	...
	private ConnectionIdentifier sendMessage(Message msg, String proto,
		...
		{ // TCP
			...
			if (connections == null || !connections.containsKey(conn_id)) { // modified
				...
				addConnection(conn);
				if (!msg.isRegister())
					SipdroidReceiver.engine(SipdroidReceiver.mContext).register(); // modified
			} else {
	...
	public ConnectionIdentifier sendMessage(Message msg) {
		...
		{// modified
			...
			else
			{
				...
				{
					//android.util.Log.v("UA", "D3");
					SipURL url = msg.getRequestLine().getAddress();
					dest_addr = url.getHost();
					dest_port = url.getPort();
					if(url.hasMaddr())
					{
						//android.util.Log.v("UA", "D4");
		...
		//android.util.Log.w("register", "send message to " + dest_addr + ":" + dest_port + " @" + proto + " --> " + msg.toString());
	...
	protected void processReceivedMessage(Message msg) {
		try { // logs
			/*android.util.Log.v("UA", msg.getMethodId().toString() + ", " +
											//msg.getTransportProtocol() + ", " +
											msg.getRemoteAddress() + ", " +
											msg.getFromHeader().toString() + ", " +
											msg.getToHeader().toString());// + ", " +
											//msg.getBody());//fym*/
	...
	public void onReceivedMessage(Transport transport, Message msg) {
		if (pm == null) {
			pm = (PowerManager) SipdroidReceiver.mContext.getSystemService(Context.POWER_SERVICE);
			wl = pm.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, "Sipdroid.SipProvider");
		}
		wl.acquire(); // modified
		android.util.Log.v("AMR", "acquire 2");
	...
	public void onTransportTerminated(Transport transport, Exception error)
	{
		...
		if (transport.getProtocol().equals(PROTO_TCP))
		{
			...
			if(Sipdroid.on(SipdroidReceiver.mContext))
			{
				SipdroidReceiver.engine(SipdroidReceiver.mContext).register(); // modified
				System.out.println("onTransportTerminated");
			}
	...
	private final void printLog(String str, int level) {
		/*fym if (Sipdroid.release) return;
		if (event_log != null) {
	...
	{
		/*fym if (log_all_packets || len >= MIN_MESSAGE_LENGTH) {
			if (message_log != null) {
