...
import com.nercms.schedule.sip.stack.sip.transaction.*;
import com.nercms.schedule.sip.stack.sip.message.*;
import com.nercms.schedule.sip.stack.sip.header.*;
import com.nercms.schedule.sip.stack.sip.provider.*;
...
public class InviteDialog extends Dialog implements TransactionClientListener,
	...
	public void invite(String callee, String caller, String contact,
		...
		//android.util.Log.v("UA", "invite: " + session_descriptor);
		...
		//android.util.Log.v("UA", "invite msg session_descriptor: " + session_descriptor.toString());
	...
	public void invite(Message invite) {
		...
		//android.util.Log.v("UA", "invitedialog invite");
	...
	public void respond(int code, String reason, String contact, String sdp) {
		...
		if (statusIs(D_INVITED) || statusIs(D_ReINVITED)) {
			...
			resp.setBody(sdp);
			respond(resp);
			//android.util.Log.v("UA", "respond " + resp.toString());
		} else
	...
	public void onReceivedMessage(SipProvider sip_provider, Message msg) {
		...
		if (msg.isRequest() && msg.isBye()) {
			...
			Message resp = MessageFactory.createResponse(msg, 200, SipResponses
					.reasonOf(200), null);
			respond(resp);
			//android.util.Log.v("UA", "onReceivedMessage onDlgBye " + msg.toString());
